...
TEST_F(SharedStackTest, MultipleFibersShareSameBuffer) {
    ...
    auto fiber1 = std::make_shared<Fiber>([&execution_order]() {
        execution_order.push_back(1);
        Fiber::yield();
        execution_order.push_back(3);
    }, shared_stack.get(), "fiber1");
    
    auto fiber2 = std::make_shared<Fiber>([&execution_order]() {
...
TEST_F(SharedStackTest, MultipleFibersDataIntegrity) {
    ...
    auto fiber1 = std::make_shared<Fiber>([&value1]() {
        int local_val = 100;
        value1 = local_val;
        Fiber::yield();
        // 验证局部变量在resume后仍然正确
    ...
    auto fiber2 = std::make_shared<Fiber>([&value2]() {
        int local_val = 200;
        value2 = local_val;
        Fiber::yield();
        // 验证局部变量在resume后仍然正确
...
TEST_F(SharedStackTest, SharedStackComplexDataPreservation) {
    ...
    auto fiber1 = std::make_shared<Fiber>([&result1]() {
        ComplexData local;
        for (int i = 0; i < 10; ++i) {
            local.values[i] = i * 10;
        }
    ...
    auto fiber2 = std::make_shared<Fiber>([&result2]() {
        ComplexData local;
        for (int i = 0; i < 10; ++i) {
...
TEST_F(SharedStackTest, SharedStackRecursion) {
    ...
    auto fiber1 = std::make_shared<Fiber>([&result1, &fibonacci]() {
        result1 = fibonacci(10);
        Fiber::yield();
        result1 = fibonacci(15);
    }, shared_stack.get());
    
    auto fiber2 = std::make_shared<Fiber>([&result2, &fibonacci]() {
...
TEST_F(SharedStackTest, MixedStackModes) {
    ...
    // 共享栈协程
    auto shared_fiber1 = std::make_shared<Fiber>([&order]() {
        order.push_back(1);
        Fiber::yield();
        order.push_back(4);
    }, shared_stack.get());
    ...
    // 共享栈协程
    auto shared_fiber2 = std::make_shared<Fiber>([&order]() {
        order.push_back(3);
        Fiber::yield();
        order.push_back(6);
    }, shared_stack.get());
...
TEST_F(SharedStackTest, ThreadLocalStackConfig) {
    ...
    for (int t = 0; t < thread_count; ++t) {
        threads.emplace_back([t, &shared_count, &independent_count]() {
            ...
            auto fiber = std::make_shared<Fiber>([t, &shared_count, &independent_count]() {
                if (t % 2 == 0) {
                    shared_count++;
                } else {
                    independent_count++;
    ...
    for (int t = 0; t < thread_count; ++t) {
        threads.emplace_back([&total_count, fibers_per_thread]() {
            ...
            for (int i = 0; i < fibers_per_thread; ++i) {
                fibers.push_back(std::make_shared<Fiber>([&local_count]() {
                    local_count++;
                    Fiber::yield();
                    local_count++;
                }, shared_stack.get()));
...
TEST_F(SharedStackTest, SharedStackFrequentSwitch) {
    ...
    auto fiber1 = std::make_shared<Fiber>([&count1]() {
        for (int i = 0; i < 100; ++i) {
            count1++;
            Fiber::yield();
        }
    ...
    auto fiber2 = std::make_shared<Fiber>([&count2]() {
        for (int i = 0; i < 100; ++i) {
            count2++;
            Fiber::yield();
        }
...
TEST_F(SharedStackTest, SchedulerIndependentStackMode) {
    // 创建使用独立栈模式的调度器
    auto scheduler = std::make_shared<Scheduler>(1, "IndependentStackScheduler", false);
