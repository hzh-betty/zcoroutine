...
// 测试1：创建协程池
TEST_F(FiberPoolTest, CreatePool) { ASSERT_NE(pool_, nullptr); }

// 测试2：从池中获取协程
TEST_F(FiberPoolTest, AcquireFiber) {
  bool executed = false;
  auto fiber = pool_->acquire([&executed]() { executed = true; });
...
TEST_F(FiberPoolTest, ReleaseNonTerminatedFiber) {
  auto fiber = pool_->acquire([]() { Fiber::yield(); });
...
TEST_F(FiberPoolTest, ReleaseNullptr) {
  EXPECT_NO_THROW({ pool_->release(nullptr); });
}
  ...
  for (int t = 0; t < thread_num; ++t) {
    threads.emplace_back([this, &total_executed, fibers_per_thread]() {
      for (int i = 0; i < fibers_per_thread; ++i) {
        auto fiber = pool_->acquire(
            [&total_executed]() { total_executed.fetch_add(1); });
        ASSERT_NE(fiber, nullptr);
        fiber->resume();
      }
    });
  ...
  for (int t = 0; t < thread_num; ++t) {
    threads.emplace_back([this, &total_ops, operations]() {
      for (int i = 0; i < operations; ++i) {
        auto fiber = pool_->acquire([&total_ops]() { total_ops.fetch_add(1); });
        fiber->resume();
        pool_->release(fiber);
      }
    });
...
TEST_F(FiberPoolTest, FiberResetAndReuse) {
  ...
  auto fiber = pool_->acquire([&first_count]() { first_count++; });
  fiber->resume();
  ...
  // 再次获取（应该复用并重置）
  auto reused = pool_->acquire([&second_count]() { second_count++; });
  reused->resume();
...
TEST_F(FiberPoolTest, ExceptionInFiber) {
  auto fiber =
      pool_->acquire([]() { throw std::runtime_error("Test exception"); });
