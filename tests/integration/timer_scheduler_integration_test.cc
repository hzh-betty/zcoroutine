...
TEST_F(TimerSchedulerIntegrationTest, TimerTriggersScheduledFiber) {
    ...
    timer_manager_->add_timer(50, [this, &executed]() {
        scheduler_->schedule([&executed]() {
            executed.store(true);
        });
    }, false);
...
TEST_F(TimerSchedulerIntegrationTest, RecurringTimerSchedulesTasks) {
    ...
    timer_manager_->add_timer(30, [this, &count]() {
        scheduler_->schedule([&count]() {
            count.fetch_add(1);
        });
    }, true);
...
TEST_F(TimerSchedulerIntegrationTest, MultipleTimersScheduleDifferentTasks) {
    ...
    timer_manager_->add_timer(30, [this, &task1_count]() {
        scheduler_->schedule([&task1_count]() { task1_count.fetch_add(1); });
    }, false);
    
    timer_manager_->add_timer(50, [this, &task2_count]() {
        scheduler_->schedule([&task2_count]() { task2_count.fetch_add(1); });
    }, false);
    
    timer_manager_->add_timer(70, [this, &task3_count]() {
...
TEST_F(TimerSchedulerIntegrationTest, CancelTimerStopsScheduling) {
    ...
    auto timer = timer_manager_->add_timer(30, [this, &count]() {
        scheduler_->schedule([&count]() {
            count.fetch_add(1);
        });
    }, true);
...
TEST_F(TimerSchedulerIntegrationTest, TimerSchedulesFiberWithYield) {
    ...
    timer_manager_->add_timer(50, [this, &step]() {
...
TEST_F(TimerSchedulerIntegrationTest, HighFrequencyTimerScheduling) {
    ...
    // 10ms的高频定时器
    timer_manager_->add_timer(10, [this, &count]() {
        scheduler_->schedule([&count]() {
            count.fetch_add(1);
        });
    }, true);
...
TEST_F(TimerSchedulerIntegrationTest, MassiveTimersScheduling) {
    ...
    for (int i = 0; i < timer_count; ++i) {
        timer_manager_->add_timer(50 + (i % 50), [this, &completed]() {
...
TEST_F(TimerSchedulerIntegrationTest, TimerResetAndReschedule) {
    ...
    auto timer = timer_manager_->add_timer(100, [this, &count]() {
        scheduler_->schedule([&count]() {
            count.fetch_add(1);
        });
    }, false);
...
TEST_F(TimerSchedulerIntegrationTest, MixedOneshotAndRecurringTimers) {
    ...
    // 一次性定时器
    for (int i = 0; i < 5; ++i) {
        timer_manager_->add_timer(50, [this, &oneshot_count]() {
            scheduler_->schedule([&oneshot_count]() {
                oneshot_count.fetch_add(1);
            });
        }, false);
    }
    ...
    // 循环定时器
    for (int i = 0; i < 2; ++i) {
        timer_manager_->add_timer(50, [this, &recurring_count]() {
            scheduler_->schedule([&recurring_count]() {
                recurring_count.fetch_add(1);
            });
        }, true);
    }
