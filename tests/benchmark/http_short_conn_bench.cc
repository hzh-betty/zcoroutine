...
void accept_loop(int listen_fd, IoScheduler::ptr scheduler) {
  ...
  while (g_running.load(std::memory_order_relaxed)) {
    ...
    // 调度处理协程
    scheduler->schedule([client_fd]() { handle_client(client_fd); });
  }
}

// 客户端发送请求
void client_worker(int request_count) {
  set_hook_enable(true);
...
void print_results(const char *stack_mode) {
  ...
  std::cout << "========== HTTP Short Connection Benchmark ==========" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < client_count; i++) {
    scheduler->schedule([requests_per_client]() { client_worker(requests_per_client); });
  }
