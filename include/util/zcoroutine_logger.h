...
#define ZCOROUTINE_LOG_DEBUG(fmt, ...)                                         \
  zlog::getLogger("zcoroutine_logger")->ZLOG_DEBUG(fmt, ##__VA_ARGS__)
#define ZCOROUTINE_LOG_INFO(fmt, ...)                                          \
  zlog::getLogger("zcoroutine_logger")->ZLOG_INFO(fmt, ##__VA_ARGS__)
#define ZCOROUTINE_LOG_WARN(fmt, ...)                                          \
  zlog::getLogger("zcoroutine_logger")->ZLOG_WARN(fmt, ##__VA_ARGS__)
#define ZCOROUTINE_LOG_ERROR(fmt, ...)                                         \
  zlog::getLogger("zcoroutine_logger")->ZLOG_ERROR(fmt, ##__VA_ARGS__)
#define ZCOROUTINE_LOG_FATAL(fmt, ...)                                         \
  zlog::getLogger("zcoroutine_logger")->ZLOG_FATAL(fmt, ##__VA_ARGS__)
