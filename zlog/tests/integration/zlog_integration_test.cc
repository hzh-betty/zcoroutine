...
TEST_F(ZlogIntegrationTest, SyncLoggerEndToEnd) {
  ...
  logger->logImpl(LogLevel::value::WARNING, __FILE__, __LINE__, "warning message");
...
TEST_F(ZlogIntegrationTest, AsyncLoggerEndToEnd) {
  ...
  
  {
    ...
    logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, "async message 1");
    logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, "async message 2");
    logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, "async message 3");
...
TEST_F(ZlogIntegrationTest, AsyncLoggerSafeMode) {
  ...
  std::string content = readFile(logFile);
  for (int i = 0; i < 100; i++) {
    EXPECT_THAT(content, ::testing::HasSubstr("safe message " + std::to_string(i)));
  }
}
...
TEST_F(ZlogIntegrationTest, MultithreadedSyncLogger) {
  ...
  for (int t = 0; t < 4; t++) {
    threads.push_back(std::thread([&logger, &counter, t]() {
      for (int i = 0; i < 100; i++) {
        logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, 
                        ("thread" + std::to_string(t) + "_msg" + std::to_string(i)).c_str());
        counter++;
      }
    }));
  }
  ...
  
  {
    ...
    for (int t = 0; t < 4; t++) {
      threads.push_back(std::thread([&logger, t]() {
        for (int i = 0; i < 100; i++) {
          logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, 
                          ("async_t" + std::to_string(t) + "_m" + std::to_string(i)).c_str());
        }
      }));
    }
...
TEST_F(ZlogIntegrationTest, GlobalLoggerBuilder) {
  ...
  logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, "global logger test");
...
TEST_F(ZlogIntegrationTest, CompleteFormatIntegration) {
  ...
  builder.buildLoggerType(LoggerType::LOGGER_SYNC);
  builder.buildLoggerLevel(LogLevel::value::DEBUG);
  builder.buildLoggerFormatter("[%d{%Y-%m-%d %H:%M:%S}][%t][%c][%f:%l][%p]%T%m%n");
  builder.buildLoggerSink<FileSink>(logFile);
  ...
  std::regex pattern(R"(\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\]\[[^\]]+\]\[format_test\]\[test\.cc:100\]\[INFO\]\s+formatted message\n)");
...
TEST_F(ZlogIntegrationTest, StressTestSync) {
  ...
  for (int i = 0; i < count; i++) {
    logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, "stress test message");
  }
  ...
  std::cout << "Sync stress test: " << count << " messages in " 
...
TEST_F(ZlogIntegrationTest, StressTestAsync) {
  ...
  
  {
    ...
    const int count = 10000;
    for (int i = 0; i < count; i++) {
      logger->logImpl(LogLevel::value::INFO, __FILE__, __LINE__, "async stress test");
    }
