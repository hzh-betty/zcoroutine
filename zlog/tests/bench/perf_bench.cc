...
struct Config {
  ...
  Config() : mode("both"), count(1000000), threads(4), 
             duration(0), msgSize(128), outputDir("perf_bench_logs") {}
};
...
void printConfig(const Config& cfg) {
  ...
  std::cout << "Duration:    " << (cfg.duration > 0 ? std::to_string(cfg.duration) + "s" : "N/A") << "\n";
...
void printStats(const std::string& name, const Stats& stats, int msgSize) {
  ...
  double throughputKB = (stats.successCount.load() * msgSize) / seconds / 1024.0;
  ...
  std::cout << "Throughput:     " << std::fixed << std::setprecision(0) << throughputMsg << " msg/s";
  
  if (throughputMB >= 1.0) {
    std::cout << " (" << std::fixed << std::setprecision(2) << throughputMB << " MB/s)";
  } else {
    std::cout << " (" << std::fixed << std::setprecision(2) << throughputKB << " KB/s)";
  }
  std::cout << "\n";
...
void runSyncBenchmark(const Config& cfg) {
  ...
  Formatter::ptr formatter = std::make_shared<Formatter>("[%d{%H:%M:%S}][%t][%p] %m%n");
  ...
  for (int t = 0; t < cfg.threads; t++) {
    threads.push_back(std::thread([&logger, &msg, &stats, &running, countPerThread]() {
...
void runAsyncBenchmark(const Config& cfg, AsyncType asyncType) {
  std::string typeName = (asyncType == AsyncType::ASYNC_SAFE) ? "Safe" : "Unsafe";
  std::cout << "\n[2] Running Async Logger Benchmark (" << typeName << ")...\n";
  
  ensureDir(cfg.outputDir);
  std::string logFile = cfg.outputDir + "/async_" + 
  ...
  
  {
    ...
    std::shared_ptr<AsyncLogger> logger = std::make_shared<AsyncLogger>(
        "async_bench", LogLevel::value::INFO, formatter, sinks,
        asyncType, std::chrono::milliseconds(100));
    ...
    switch (opt) {
      ...
      default:
        std::cout << "Usage: " << argv[0] << " [options]\n"
                  ...
                  << "  -d <duration>  Duration in seconds, 0 for count-based (default: 0)\n"
