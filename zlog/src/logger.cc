...
Logger::Logger(const char *loggerName, const LogLevel::value limitLevel, Formatter::ptr formatter, std::vector<LogSink::ptr> &sinks) : loggerName_(loggerName), limitLevel_(limitLevel), formatter_(std::move(formatter)), sinks_(sinks.begin(), sinks.end()) {}

void Logger::serialize(const LogLevel::value level, const char *file, const size_t line, const char *data) {
...
SyncLogger::SyncLogger(const char *loggerName, const LogLevel::value limitLevel, const Formatter::ptr &formatter, std::vector<LogSink::ptr> &sinks) : Logger(loggerName, limitLevel, formatter, sinks) {}
...
AsyncLogger::AsyncLogger(const char *loggerName, const LogLevel::value limitLevel, const Formatter::ptr &formatter, std::vector<LogSink::ptr> &sinks, AsyncType looperType, std::chrono::milliseconds milliseco) : Logger(loggerName, limitLevel, formatter, sinks), looper_(std::make_shared<AsyncLooper>(AsyncLooper::Functor{[this](const Buffer &buf) { this->reLog(buf); }}, looperType, milliseco)) {}
...
LoggerBuilder::LoggerBuilder() : loggerType_(LoggerType::LOGGER_SYNC), limitLevel_(LogLevel::value::DEBUG), looperType_(AsyncType::ASYNC_SAFE), milliseco_(std::chrono::milliseconds(3000)) {}
...
LoggerManager::LoggerManager() {
    const std::unique_ptr<zlog::LocalLoggerBuilder> builder(new zlog::LocalLoggerBuilder());
