...
public:
   ...
   * 初始化缓冲区为默认大小
   */
  Buffer() : buffer_(DEFAULT_BUFFER_SIZE), writerIdx_(0), readerIdx_(0) {}
   ...
   * @return 可读数据的指针
   */
  const char *begin() const { return &buffer_[readerIdx_]; }
   ...
   * @return 可读数据大小
   */
  size_t readAbleSize() const { return writerIdx_ - readerIdx_; }
   ...
   * @brief 重置读写位置，初始化缓冲区
   */
  void reset() { readerIdx_ = writerIdx_ = 0; }
   ...
   */
  bool empty() const { return readerIdx_ == writerIdx_; }
