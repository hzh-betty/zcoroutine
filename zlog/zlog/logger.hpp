...
public:
   ...
   * @param sinks 日志落地器列表
   */
  Logger(const char *loggerName, const LogLevel::value limitLevel,
...
protected:
   ...
   * @param data 日志数据
   */
  void serialize(const LogLevel::value level, const char *file,
  ...
  AsyncLogger(const char *loggerName, const LogLevel::value limitLevel,
      ...
      : Logger(loggerName, limitLevel, formatter, sinks),
        looper_(std::make_shared<AsyncLooper>(
            AsyncLooper::Functor{
                [this](const Buffer &buf) { this->reLog(buf); }},
...
public:
   ...
   */
  LoggerBuilder()
      : loggerType_(LoggerType::LOGGER_SYNC),
        limitLevel_(LogLevel::value::DEBUG), looperType_(AsyncType::ASYNC_SAFE),
   ...
   * @brief 启用非安全异步模式
   */
  void buildEnalleUnSafe() { looperType_ = AsyncType::ASYNC_UNSAFE; }

  /**
   * @brief 设置日志器名称
   * @param loggerName 日志器名称
   */
  void buildLoggerName(const char *loggerName) { loggerName_ = loggerName; }
...
public:
   ...
   * @return 根日志器智能指针
   */
  Logger::ptr rootLogger() { return rootLogger_; }
...
public:
  ...
  Logger::ptr build() override {
    ...
    if (loggerType_ == LoggerType::LOGGER_ASYNC) {
      logger =
          std::make_shared<AsyncLogger>(loggerName_, limitLevel_, formatter_,
                                        sinks_, looperType_, milliseco_);
    } else {
      logger = std::make_shared<SyncLogger>(loggerName_, limitLevel_,
                                            formatter_, sinks_);
    }
