#include "zlog.h"
#include <iostream>
void bench(const std::string &loggerName, size_t threadNum, size_t messNum,
           size_t messLen) {
  // 1. 获取日志器
  ...
  std::cout << "\t测试日志：线程" << threadNum << "个 " << "日志数量" << messNum
            << "条 总大小" << (messNum * messLen) / 1024 << "KB" << std::endl;
  // 2. 构建指定长度的字符串
...
void syncBench(size_t threadNum) {
  std::unique_ptr<zlog::GlobalLoggerBuilder> builder(
      new zlog::GlobalLoggerBuilder());
  ...
  // builder->buildLoggerSink<zlog::RollBySizeSink>("./logfile/sync.log", 1 *
  // 1024 * 1024);
