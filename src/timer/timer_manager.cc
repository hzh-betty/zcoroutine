...
namespace zcoroutine {
    ...
    Timer::ptr TimerManager::add_timer(uint64_t timeout, std::function<void()> callback, bool recurring) {
        ZCOROUTINE_LOG_DEBUG("TimerManager::add_timer called, timeout={}ms, recurring={}", timeout, recurring);
        Timer::ptr timer(new Timer(timeout, callback, recurring, this));
        insert_timer(timer);
        ZCOROUTINE_LOG_DEBUG("TimerManager::add_timer succeeded, total_timers={}", timers_.size());
        return timer;
    }

    Timer::ptr TimerManager::add_condition_timer(uint64_t timeout, std::function<void()> callback,
                                                 std::weak_ptr<void> weak_cond, bool recurring) {
        ZCOROUTINE_LOG_DEBUG("TimerManager::add_condition_timer called, timeout={}ms, recurring={}", timeout,
                             recurring);
        ...
        Timer::ptr timer(new Timer(timeout, std::move(wrapper_callback), recurring, this));
        insert_timer(timer);
    ...
    std::vector<std::function<void()> > TimerManager::list_expired_callbacks() {
        ...
        while (!timers_.empty() && (rollback || (*timers_.begin())->next_time_ <= now)) {
        ...
        ZCOROUTINE_LOG_DEBUG("TimerManager::list_expired_callbacks completed, count={}", callbacks.size());
        return callbacks;
    }
    ...
    void TimerManager::insert_timer(Timer::ptr timer) {
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            at_front = timers_.empty() || (timer->next_time_ < (*timers_.begin())->next_time_);
    ...
    bool TimerManager::detect_clock_rollback() {
        ...
        if (now < last_time_ && (last_time_ - now) > 60 * 60 * 1000) {
            rollback = true;
            ZCOROUTINE_LOG_WARN("Clock rollback detected, now={}, last_time={}", now, last_time_);
        }
        last_time_ = now;
        return rollback;
    }
