...
FiberPool::FiberPool(size_t min_size, size_t max_size)
    : min_size_(min_size), max_size_(max_size), total_created_(0),
      total_reused_(0) {

  ZCOROUTINE_LOG_INFO("FiberPool created: min_size={}, max_size={}", min_size,
                      max_size);
}
...
FiberPool::~FiberPool() {
  ...
  clear();
  ZCOROUTINE_LOG_INFO("FiberPool destroyed: total_created={}, total_reused={}, "
                      "final_idle_count={}",
...
Fiber::ptr FiberPool::acquire(std::function<void()> func) {
  ...
  if (!idle_fibers_.empty()) {
    ...
    ZCOROUTINE_LOG_DEBUG("FiberPool::acquire from pool: fiber_id={}, "
                         "idle_remaining={}, total_reused={}",
  ...
  ZCOROUTINE_LOG_DEBUG(
      "FiberPool::acquire pool empty, need create new fiber: total_created={}",
      created);
...
void FiberPool::release(const Fiber::ptr& fiber) {
  ...
  if (fiber->state() != Fiber::State::kTerminated) {
    ZCOROUTINE_LOG_WARN(
        "FiberPool::release fiber not terminated: fiber_id={}, state={}",
  ...
  if (idle_fibers_.size() >= max_size_) {
    ZCOROUTINE_LOG_DEBUG("FiberPool::release pool full, discard fiber: "
                         "fiber_id={}, pool_size={}, max_size={}",
  ...
  ZCOROUTINE_LOG_DEBUG(
      "FiberPool::release fiber returned to pool: fiber_id={}, idle_count={}",
...
void FiberPool::resize(size_t size) {
  ...
  ZCOROUTINE_LOG_INFO(
      "FiberPool::resize: old_max={}, new_max={}, removed={}, idle_count={}",
...
void FiberPool::clear() {
  ...
  idle_fibers_.clear();
  ZCOROUTINE_LOG_INFO("FiberPool::clear: cleared {} idle fibers",
                      cleared_count);
}
...
PoolStatistics FiberPool::get_statistics() const {
  ...
  ZCOROUTINE_LOG_DEBUG(
      "FiberPool::get_statistics: created={}, reused={}, idle={}",
