...
FdContextTable::FdContextTable(size_t initial_capacity) {
    ...
    ZCOROUTINE_LOG_DEBUG("FdContextTable created with capacity={}", initial_capacity);
}
...
FdContext::ptr FdContextTable::expand_and_create(int fd) {
    ...
    if (static_cast<size_t>(fd) >= contexts_.size()) {
        ...
        size_t new_size = std::max(static_cast<size_t>(fd + 1), old_size + old_size / 2);
        contexts_.resize(new_size);
        ZCOROUTINE_LOG_DEBUG("FdContextTable expanded from {} to {}", old_size, new_size);
    }
